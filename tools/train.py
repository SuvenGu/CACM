...
def main():
    ...
    # Data loading code
    traindir =[]
    for i in config.DATASET.TRAIN_DATASET:
        traindir.append(os.path.join(args.dataDir, i+"_points",config.DATASET.TRAIN_SET+"_"+i+".csv"))

    valdir =[]
    for i in config.DATASET.VAL_DATASET:
        valdir.append(os.path.join(args.dataDir, i+"_points",config.DATASET.VAL_SET+"_"+i+".csv"))

    train_dataset = CropAttriMappingDataset(traindir,c_dim = config.MODEL.C_DIM,T=config.MODEL.T,T_a=config.MODEL.T_A)
